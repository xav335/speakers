...
require 'classes/Newspro.php';
	...
	$newspro = new Newspro();
	$result = $newspro->newsproGet(null);
	//print_r($result);
	if (empty($result)) {
					...
					<tbody>
						...
						if (!empty($result)) {
							...
							<tr class="<?php if ($i%2!=0) echo 'info'?>">
								...
								<td><a href="newspro-edit.php?id=<?php echo $value['id_news'] ?>"><img src="img/modif.png" width="30" alt="Modifier" ></a></td>
								<td>
									<div style="display: none;" class="supp<?php echo $value['id_news']?> alert alert-warning alert-dismissible fade in" role="alert">
								      ...
								      <button type="button" class="btn btn-danger" onclick="location.href='formprocess.php?reference=newspro&action=delete&id=<?php echo $value['id_news'] ?>'">Oui !</button>
								 	</div>
