...
if (!empty($_GET)){ //Modif 
	...
	$result = $goldbookpro->goldbookproGet($_GET['id']);
	//print_r($result);
	if (empty($result)) {
		$message = 'Aucun enregistrements';
	} else {
		$labelTitle= 	'Livre d\' or N°: '. $_GET['id'];
		$date_goldbookpro= traitement_datetime_affiche($result[0]['date']);
		$id= 			$_GET['id'];
		$nom=  			$result[0]['nom'];
...
} else { //ajout goldbookpro
	$action = 'add';
	$labelTitle = 'Edition Livre Or ';
	$id= 			null;
	$nom=  			null;
	$email= 		null;
	$date_goldbookpro= null;
	$message= 		null;
	$online = 		null;
}
?>
		...
		<div class="row">
			...
			<div class="col-xs-12 col-sm-12 col-md-12">
					...
					<form name="formulaire" class="form-horizontal" method="POST"  action="formprocess.php">
						<input type="hidden" name="reference" value="goldbookpro">
						<input type="hidden" name="action" value="<?php echo $action ?>">
						<input type="hidden" name="id" id="id" value="<?php echo $id ?>">
						
						<div class="form-group" >
							<label class="col-sm-1" >Date :</label>
						    <input class="col-sm-2" type="text" name="datepicker" required id="datepicker" value="<?php echo $date_goldbookpro?>" >
						</div>
						<div class="form-group" >
