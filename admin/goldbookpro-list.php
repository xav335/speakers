...
require 'classes/Goldbookpro.php';
	...
	$result = $goldbookpro->goldbookproGet(null);
	//print_r($result);
	if (empty($result)) {
					...
					<tbody>
						...
						if (!empty($result)) {
							...
							<tr class="<?php if ($i%2!=0) echo 'info'?>">
								...
								<td><a href="goldbookpro-edit.php?id=<?php echo $value['id'] ?>"><img src="img/modif.png" width="30" alt="Modifier" ></a></td>
								<td>
									<div style="display: none;" class="supp<?php echo $value['id']?> alert alert-warning alert-dismissible fade in" role="alert">
								      ...
								      <button type="button" class="btn btn-danger" onclick="location.href='formprocess.php?reference=goldbookpro&action=delete&id=<?php echo $value['id'] ?>'">Oui !</button>
								 	</div>
